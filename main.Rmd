...
Here the residuals are distributed more or less equally arounf the mean and we can`t observe a trend that the variance increases as the value increase, so heteroscedasticity is also absent here. 
As we can see our model suggest quite a good fit, but I still will try to improve it.

### Time-Series Evolution Vizualization
```{r}
#load more data
...
```
We see how happiness rate changed over years and I suggest examine the most interesting cases seperately.
We see a big decline of happiness score in Austria in 2017 which was then followed by dramatic increase.
Also, United States has completely missing data in the time range between 2016 and 2018 which means that its value dropped under 7.0.
...
From now on, we can state that GDP and happiness score are not perfectly correlated and it does not always explain the happiness rate in country. While health almost always has the same curve as happiness rate.
